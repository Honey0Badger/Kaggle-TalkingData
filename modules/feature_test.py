...
def read_merge_process5(ftrain, ftest=None):
    ...
    train_df = do_os_click_freq( train_df ); gc.collect()
    train_df = do_app_os_click_freq( train_df ); gc.collect()
    train_df = do_app_dev_click_freq( train_df ); gc.collect()
    train_df = do_chn_os_click_freq( train_df ); gc.collect()
    train_df = do_chn_dev_click_freq( train_df ); gc.collect()
...
def do_os_click_freq( train_df ):
    gp = train_df[['ip', 'os']].groupby(by=['os'])[['ip']]\
            .agg(lambda x: float(len(x)) / len(x.unique())).reset_index()\
            .rename(index=str, columns={'ip': 'os_click_freq'})
    train_df = train_df.merge(gp, on=['os'], how='left')
    del gp
    gc.collect()
    return train_df

def do_app_os_click_freq( train_df ):
    gp = train_df[['ip', 'app', 'os']].groupby(by=['app', 'os'])[['ip']]\
            .agg(lambda x: float(len(x)) / len(x.unique())).reset_index()\
            .rename(index=str, columns={'ip': 'app_os_click_freq'})
    train_df = train_df.merge(gp, on=['app', 'os'], how='left')
    del gp
    gc.collect()
    return train_df

def do_chn_os_click_freq( train_df ):
    gp = train_df[['ip', 'channel', 'os']].groupby(by=['channel', 'os'])[['ip']]\
            .agg(lambda x: float(len(x)) / len(x.unique())).reset_index()\
            .rename(index=str, columns={'ip': 'chn_os_click_freq'})
    train_df = train_df.merge(gp, on=['channel', 'os'], how='left')
    del gp
    gc.collect()
    return train_df

def do_chn_dev_click_freq( train_df ):
    gp = train_df[['ip', 'channel', 'device']].groupby(by=['channel', 'device'])[['ip']]\
            .agg(lambda x: float(len(x)) / len(x.unique())).reset_index()\
            .rename(index=str, columns={'ip': 'chn_dev_click_freq'})
    train_df = train_df.merge(gp, on=['channel', 'device'], how='left')
    del gp
    gc.collect()
    return train_df

def do_app_dev_click_freq( train_df ):
    gp = train_df[['ip', 'app', 'device']].groupby(by=['app', 'device'])[['ip']]\
            .agg(lambda x: float(len(x)) / len(x.unique())).reset_index()\
            .rename(index=str, columns={'ip': 'app_dev_click_freq'})
    train_df = train_df.merge(gp, on=['app', 'device'], how='left')
    del gp
    gc.collect()
    return train_df

...
full_df = pd.read_pickle('../modules/21_feature_bot60m_data.pkl')
...
kcv = StratifiedKFold(n_splits=4, shuffle=True, random_state=42)

Bayes_search(full_df[predictors], full_df[target], lgbm_est_base, lgbm_param_space, cv=kcv, iterations=20, n_jobs=3, refit=True)
